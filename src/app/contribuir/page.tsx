    ...
    return (
      <div className="p-6 max-w-lg mx-auto space-y-6 pt-8">
        <div className="text-center space-y-2">
          ...
          <p className="text-sm text-text-secondary">Ajude outros pais com informações reais.</p>
        </div>
    ...
    return (
      <div className="flex-1 flex items-center justify-center p-6">
        <div className="w-full max-w-sm text-center space-y-4">
          ...
          <p className="text-sm text-text-secondary">Sua contribuição foi salva de forma anônima.</p>
  ...
  return (
    <div className="p-6 max-w-lg mx-auto space-y-6 pt-8">
      <div className="text-center space-y-1">
        ...
        <p className="text-sm text-text-secondary">Informe os valores de uma escola.</p>
      </div>

      <form onSubmit={handleSubmit} className="card space-y-4">
        {/* School search */}
        <div className="relative">
          <label className="text-xs font-medium text-text-secondary block mb-1">Escola</label>
          ...
          {escolasSugestoes.length > 0 && (
            <ul className="absolute z-10 top-full mt-1 w-full bg-surface border border-border rounded-xl shadow-lg overflow-hidden">
              {escolasSugestoes.map((e) => (
                <li key={e.id} onClick={() => handleSelect(e)}
                  className="px-4 py-2.5 text-sm cursor-pointer hover:bg-surface-hover border-b border-border last:border-0">
        ...
        <div>
          <label className="text-xs font-medium text-text-secondary block mb-1">Série / Ano</label>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
          ...
          <div>
            <label className="text-xs font-medium text-text-secondary block mb-1">Matrícula (R$)</label>
            <input className="input-field" type="number" step="0.01" min="0" placeholder="0,00" value={matricula}
              onChange={(e) => setMatricula(e.target.value)} />
          </div>
          <div>
            <label className="text-xs font-medium text-text-secondary block mb-1">Material (R$)</label>
            <input className="input-field" type="number" step="0.01" min="0" placeholder="0,00" value={material}
              onChange={(e) => setMaterial(e.target.value)} />
          </div>
        </div>

        {error && <p className="text-sm text-danger bg-red-50 dark:bg-red-900/20 rounded-lg p-3">{error}</p>}
